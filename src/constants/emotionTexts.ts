...
export const ISLAND_DESCRIPTIONS: Record<number, string> = {
  1: '00님이 발견한 가장 소중한 감정 보물은 우정이에요.<br/>우리는 지금 우정 섬에 도착했어요.<br/>우정이란 감정은 어색함 없는 침묵과 반복되는 웃음 속에서 자라나요. <br/>우리는 친구와 함께일 때 더 대담하고 솔직해지며 <br/>가장 나다운 모습으로 살아 갈 수 있어요.<br/><br/>00님의 소중한 감정에 대한 글을 유리병을 클릭해 다시 확인해보세요.', // 우정섬 설명
  5: '00님이 발견한 가장 소중한 감정 보물은 분노예요.<br/>우리는 지금 분노 섬에 도착했어요.<br/>부당함 앞에서 나를 지키려 솟구치는 불꽃이 보이네요. <br/>불편하고 거친 감정이지만 그 안에는 정당함, 변화에 대한 열망이 깃들어 <br/>결국 전환으로 나아가도록 해요.<br/><br/>00님의 소중한 감정에 대한 글을 유리병을 클릭해 다시 확인해보세요.', // 분노섬 설명
  7: '00님이 발견한 가장 소중한 감정 보물은 사랑이에요.<br/>우리는 지금 사랑 섬에 도착했어요.<br/>사랑이란 감정은 마음을 열게 하고 설렘과 불안을 함께 불러오기도 하지만 <br/>그 모든 면을 끌어안았을 때, 사랑이 비로소 00님의 것이 되어요.<br/><br/>00님의 소중한 감정에 대한 글을 유리병을 클릭해 다시 확인해보세요.', // 사랑섬 설명
  8: '00님이 발견한 가장 소중한 감정 보물은 상실감이에요. <br/>우리는 지금 상실섬에 도착했어요.<br/>잃어버린 것과 함께 살아가는 법을 가르치고, 사라진 관계와 시간 그리고<br/>닿을 수 없는 순간들은 각기 다른 얼굴로 남아요.<br/>상실은 끝이 아니라 다른 모습으로 머무는 시작이에요.<br/><br/>00님의 소중한 감정에 대한 글을 유리병을 클릭해 다시 확인해보세요.', // 상실섬 설명
  9: '00님이 발견한 가장 소중한 감정 보물은 방어이에요.<br/>우리는 지금 방어섬에 도착했어요.<br/>공격을 막기위해 시작되지만 믿음이 흔들릴 때 또는 이해받지 못할까 <br/>두렵기 때문에 계속되는 감정, 그 안에는 ‘나는 틀리지 않았다’는<br/>간절한 바람이 숨어 있어요.<br/><br/>00님의 소중한 감정에 대한 글을 유리병을 클릭해 다시 확인해보세요.', // 방어섬 설명
};

export const getIslandDescription = (emotionNumber: number, username?: string | null): string => {
  const raw = ISLAND_DESCRIPTIONS[emotionNumber] || '';
  if (username) {
    return raw.split('00').join(username);
  }
  return raw;
};
